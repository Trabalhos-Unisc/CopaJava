...
public class Main {
    ...
    static void menuInicial() {
        ...
        switch (op) {
            case "1" -> entrarComoParticipante();
            case "2" -> entrarComoAdmin();
            case "0" -> { System.out.println("Até logo!"); System.exit(0); }
    ...
    static void menuAdmin() {
        ...
        switch (op) {
            case "1" -> adminAdicionarParticipante();
            case "2" -> adminCadastrarSelecao();
            case "3" -> adminCadastrarJogador();
            case "4" -> adminCadastrarJogo();
            case "5" -> adminAbrirJogo();
            case "6" -> adminRegistrarResultado();
            case "7" -> exibirRanking();
            case "8" -> listarJogos();
            case "0" -> { modoAdmin = false; System.out.println(" Logout realizado."); }
            default  -> System.out.println(" Opção inválida.");
        }
    }
    ...
    static void adminCadastrarJogador() {
        ...
        Posicao posicao = switch (pos) {
            case 1 -> new PosicaoAtacante();
            case 2 -> new PosicaoMeioCampista();
            case 3 -> new PosicaoDefensor();
            case 4 -> new PosicaoGoleiro();
            default -> null;
        };
    ...
    static void adminAbrirJogo() {
        List<Jogo> agendados = admin.getJogos().stream()
                .filter(j -> j.getStatus() == StatusJogo.AGENDADO).toList();
    ...
    static void adminRegistrarResultado() {
        List<Jogo> abertos = admin.getJogos().stream()
                .filter(j -> j.getStatus() == StatusJogo.ABERTO_PARA_PALPITES).toList();
